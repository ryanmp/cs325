...
\textbf{Pre-explanation:} 

The divide and conquer algorithm works by splitting the array in half, finding the middle of the two smaller arrays, and then proceeding to do so until it hits the base case - n = 1. While doing this it finds the maximum subarray of each smaller array and then combines to get the maximum subarray of the entire array.

\textbf{Proof by contradiction:}

Base Case: size of array = 1

With an array of size 1, there is only one possible subarray, which our program will return correctly.
...
A maximum subarray could not possible have a negative sum. Our algorithm starts from the first item and tracks the sum along the way. If the sum at some point becomes negative, it means the whole sequence can contain the biggest sum but cannot be a part of anything bigger.
